...
export default class QuizUpdate extends Component {
    ...
    constructor(props) {
      ...
      componentDidMount(){
        ...
        const req02=axios.get(`http://localhost:8070/quiz/get/${id}`);
        ...
        axios.all([req01,req02]).then(axios.spread((res1,res2) => {
            ...
            console.log(this.state.Attempts_Ass,this.state.FromDate);
        }));
                    ...
                    {this.state.Attempts_Ass.map((attempts_Ass)=>(
                            <tr class="table-secondary">
                                ...
                                <td><div className="d-grid gap-2 mx-auto">
                                    <Link className="btn btn-warning" to={`/i/updateresults/${attempts_Ass._id}`} role="button">Update the results</Link>
                                </div></td>
                                <td><div className="d-grid gap-2 mx-auto">
